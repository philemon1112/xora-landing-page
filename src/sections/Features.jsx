            ...
            <Element name="features">
                <div className="container">
                    <div className="relative flex md:flex-wrap flex-nowrap border-2 border-s3 rounded-7xl md:overflow-hidden max-md:flex-col feature-after md:g7 max-md:border-none max-md:rounded-none max-md:gap-3">
                        {features.map(({ icon, caption, title, text, button }, index) => (
                            <div key={index} className="relative z-2 md:px-10 px-5 md:pb-10 pb-5 flex-50 max-md:g7 max-md:border-2 max-md:border-s3 max-md:rounded-3xl max-md:flex-320 ">
                                ...
                                <h2 className='max-w-400 mb-7 text-p4 max-md:mb-6 max-md:h5 '>{title}</h2>
                        ...
                        <ul className="relative flex justify-around flex-grow px-[5%] border-2 border-s3 rounded-7xl max-md:hidden">
                            <div className='absolute bg-s3/20 top-[38%] lft-0 right-0 w-full h-[1px] z-10' />
                            {details.map(({ title, icon }, index) => (
                                <li key={index} className="relative pt-16 px-4 pb-14">
